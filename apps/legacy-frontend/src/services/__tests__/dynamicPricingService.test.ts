...
describe('Dynamic Pricing Service - Business Logic', () => {
  describe('getCurrentSeason', () => {
    it('should return SPRING for March-May', () => {
      expect(
        dynamicPricingService.getCurrentSeason(new Date('2025-03-15'))
      ).toBe('SPRING');
      expect(
        dynamicPricingService.getCurrentSeason(new Date('2025-04-15'))
      ).toBe('SPRING');
      expect(
        dynamicPricingService.getCurrentSeason(new Date('2025-05-15'))
      ).toBe('SPRING');
    });

    it('should return SUMMER for June-August', () => {
      expect(
        dynamicPricingService.getCurrentSeason(new Date('2025-06-15'))
      ).toBe('SUMMER');
      expect(
        dynamicPricingService.getCurrentSeason(new Date('2025-07-15'))
      ).toBe('SUMMER');
      expect(
        dynamicPricingService.getCurrentSeason(new Date('2025-08-15'))
      ).toBe('SUMMER');
    });

    it('should return FALL for September-November', () => {
      expect(
        dynamicPricingService.getCurrentSeason(new Date('2025-09-15'))
      ).toBe('FALL');
      expect(
        dynamicPricingService.getCurrentSeason(new Date('2025-10-15'))
      ).toBe('FALL');
      expect(
        dynamicPricingService.getCurrentSeason(new Date('2025-11-15'))
      ).toBe('FALL');
    });

    it('should return WINTER for December-February', () => {
      expect(
        dynamicPricingService.getCurrentSeason(new Date('2025-12-15'))
      ).toBe('WINTER');
      expect(
        dynamicPricingService.getCurrentSeason(new Date('2025-01-15'))
      ).toBe('WINTER');
      expect(
        dynamicPricingService.getCurrentSeason(new Date('2025-02-15'))
      ).toBe('WINTER');
    });
  });
  ...
  describe('getDayOfWeek', () => {
    ...
    it('should handle various days of the week', () => {
      ...
      expect(dynamicPricingService.getDayOfWeek('2025-11-05')).toBe(
        'WEDNESDAY'
      );
  ...
  describe('isHoliday', () => {
    ...
    it('should return true for holiday dates', () => {
      expect(dynamicPricingService.isHoliday('2025-12-25', holidays)).toBe(
        true
      );
      expect(dynamicPricingService.isHoliday('2025-01-01', holidays)).toBe(
        true
      );
    });

    it('should return false for non-holiday dates', () => {
      expect(dynamicPricingService.isHoliday('2025-11-15', holidays)).toBe(
        false
      );
    });
  });
  ...
  describe('calculateDaysInAdvance', () => {
    it('should calculate days in advance correctly', () => {
      const days = dynamicPricingService.calculateDaysInAdvance(
        '2025-11-15',
        '2025-11-01'
      );
      expect(days).toBe(14);
  ...
  describe('applyPercentageAdjustment', () => {
    it('should calculate percentage adjustment correctly', () => {
      ...
      expect(dynamicPricingService.applyPercentageAdjustment(100, -10)).toBe(
        -10
      );
  ...
  describe('calculateTotalAdjustment', () => {
    ...
    it('should handle negative adjustments (discounts)', () => {
      ...
      const total =
        dynamicPricingService.calculateTotalAdjustment(discountAdjustments);
  ...
  describe('calculateFinalPrice', () => {
    ...
    it('should calculate final price correctly', () => {
      const finalPrice = dynamicPricingService.calculateFinalPrice(
        100,
        adjustments
      );
  ...
  describe('Business Rules', () => {
    it('should correctly identify peak season', () => {
      const summer = dynamicPricingService.getCurrentSeason(
        new Date('2025-07-04')
      );
