...
export function handleApiError(
  ...
  if (axios.isAxiosError(error)) {
    ...
    if (axiosError.response?.status === 401) {
      return {
        message:
          'Your session has expired or you are not authorized. Please login again.',
        code: 'UNAUTHORIZED',
        context,
        originalError: error,
        statusCode: 401,
    ...
    if (axiosError.response?.status === 400) {
      ...
      const errorMessage =
        details?.message || details?.error || 'Invalid request';
      return {
    ...
    if (axiosError.response) {
      const details = axiosError.response?.data as any;
      const errorMessage =
        details?.message || details?.error || axiosError.message;
      return {
        message: errorMessage,
 ...
 */
export function displayErrorToUser(
  error: unknown,
