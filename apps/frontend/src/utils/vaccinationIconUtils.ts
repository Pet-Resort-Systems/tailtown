...
export const enhancePetWithVaccinationIcons = (pet: any): any => {
  ...
  const uniqueIcons = enhancedIcons.filter(
    (icon, index) => enhancedIcons.indexOf(icon) === index
  );
 ...
 */
export const enhanceReservationsWithVaccinationIcons = (
  reservations: any[]
): any[] => {
  ...
  return reservations.map((reservation) => ({
    ...reservation,
    pet: reservation.pet
      ? enhancePetWithVaccinationIcons(reservation.pet)
      : reservation.pet,
  }));
};
