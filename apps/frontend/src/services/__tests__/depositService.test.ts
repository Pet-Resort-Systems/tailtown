...
import { DepositRule, DepositRuleConditions } from '../../types/deposit';
...
describe('Deposit Service', () => {
  describe('evaluateRuleConditions', () => {
    it('should match cost threshold conditions', () => {
      ...
      expect(
        depositService.evaluateRuleConditions(conditions, {
          totalCost: 600,
  ...
  describe('calculateDepositAmount', () => {
    it('should calculate percentage deposit', () => {
      expect(
        depositService.calculateDepositAmount(1000, 'PERCENTAGE', 50)
      ).toBe(500);
      expect(depositService.calculateDepositAmount(750, 'PERCENTAGE', 25)).toBe(
        187.5
      );
    });
    ...
    it('should calculate fixed deposit', () => {
      expect(
        depositService.calculateDepositAmount(1000, 'FIXED', undefined, 100)
      ).toBe(100);
      expect(
        depositService.calculateDepositAmount(50, 'FIXED', undefined, 100)
      ).toBe(50); // Min of fixed and total
    });
    ...
    const refundTiers = [
      {
        ...
        description: '100% if 14+ days',
      },
      {
        daysBeforeStart: 7,
        refundPercentage: 50,
        description: '50% if 7-13 days',
      },
      {
        daysBeforeStart: 0,
        refundPercentage: 0,
        description: 'No refund if <7 days',
      },
    ];

    it('should return 0 for non-refundable', () => {
      expect(
        depositService.calculateRefundAmount(
          500,
          'NON_REFUNDABLE',
          undefined,
          '2025-12-01',
  ...
  describe('formatDepositAmount', () => {
    it('should format full payment', () => {
      expect(depositService.formatDepositAmount(1000, 1000)).toBe(
        'Full payment required'
      );
    });

    it('should format partial payment with percentage', () => {
      expect(depositService.formatDepositAmount(500, 1000)).toBe(
        '$500.00 (50%)'
      );
      expect(depositService.formatDepositAmount(250, 1000)).toBe(
        '$250.00 (25%)'
      );
    });
  });
  ...
  describe('getRefundPolicyText', () => {
    it('should return correct text for each policy', () => {
      expect(depositService.getRefundPolicyText('FULL_REFUND')).toBe(
        'Fully refundable'
      );
      expect(depositService.getRefundPolicyText('PARTIAL_REFUND')).toBe(
        'Partially refundable'
      );
      expect(depositService.getRefundPolicyText('NON_REFUNDABLE')).toBe(
        'Non-refundable'
      );
      expect(depositService.getRefundPolicyText('TIERED_REFUND')).toBe(
  ...
  describe('validateRule', () => {
    ...
    it('should require deposit amount for FIXED type', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Deposit fixed amount must be greater than 0'
      );
    });
    ...
    it('should require refund tiers for TIERED_REFUND', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'Refund tiers are required for tiered refund policy'
      );
    });
  });
