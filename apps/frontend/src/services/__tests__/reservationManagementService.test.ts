...
describe('Reservation Management Service - Business Logic', () => {
  ...
  describe('canModifyReservation', () => {
    it('should allow modification for confirmed future reservations', () => {
      ...
      const canModify =
        reservationManagementService.canModifyReservation(futureReservation);
    ...
    it('should not allow modification for cancelled reservations', () => {
      ...
      const canModify =
        reservationManagementService.canModifyReservation(cancelledReservation);
    ...
    it('should not allow modification for completed reservations', () => {
      ...
      const canModify =
        reservationManagementService.canModifyReservation(completedReservation);
    ...
    it('should not allow modification for checked-in reservations', () => {
      ...
      const canModify =
        reservationManagementService.canModifyReservation(checkedInReservation);
    ...
    it('should not allow modification for no-show reservations', () => {
      ...
      const canModify =
        reservationManagementService.canModifyReservation(noShowReservation);
    ...
    it('should not allow modification within 24 hours of check-in', () => {
      ...
      const canModify =
        reservationManagementService.canModifyReservation(soonReservation);
  ...
  describe('canCancelReservation', () => {
    it('should allow cancellation for confirmed reservations', () => {
      ...
      const canCancel =
        reservationManagementService.canCancelReservation(confirmedReservation);
    ...
    it('should allow cancellation for pending reservations', () => {
      ...
      const canCancel =
        reservationManagementService.canCancelReservation(pendingReservation);
    ...
    it('should not allow cancellation for already cancelled reservations', () => {
      ...
      const canCancel =
        reservationManagementService.canCancelReservation(cancelledReservation);
    ...
    it('should not allow cancellation for completed reservations', () => {
      ...
      const canCancel =
        reservationManagementService.canCancelReservation(completedReservation);
    ...
    it('should not allow cancellation for checked-in reservations', () => {
      ...
      const canCancel =
        reservationManagementService.canCancelReservation(checkedInReservation);
  ...
  describe('calculateRefundPercentage', () => {
    it('should return 100% refund for 7+ days before check-in', () => {
      const percentage =
        reservationManagementService.calculateRefundPercentage(7);
      expect(percentage).toBe(100);

      const percentage10 =
        reservationManagementService.calculateRefundPercentage(10);
      expect(percentage10).toBe(100);
    });

    it('should return 50% refund for 3-6 days before check-in', () => {
      const percentage3 =
        reservationManagementService.calculateRefundPercentage(3);
      expect(percentage3).toBe(50);

      const percentage5 =
        reservationManagementService.calculateRefundPercentage(5);
      expect(percentage5).toBe(50);

      const percentage6 =
        reservationManagementService.calculateRefundPercentage(6);
      expect(percentage6).toBe(50);
    });

    it('should return 25% refund for 1-2 days before check-in', () => {
      const percentage1 =
        reservationManagementService.calculateRefundPercentage(1);
      expect(percentage1).toBe(25);

      const percentage2 =
        reservationManagementService.calculateRefundPercentage(2);
      expect(percentage2).toBe(25);
    });

    it('should return 0% refund for same day or past', () => {
      const percentage0 =
        reservationManagementService.calculateRefundPercentage(0);
      expect(percentage0).toBe(0);

      const percentageNegative =
        reservationManagementService.calculateRefundPercentage(-1);
  ...
  describe('formatModificationType', () => {
    it('should format modification types correctly', () => {
      expect(
        reservationManagementService.formatModificationType('DATE_CHANGE')
      ).toBe('Date Changed');

      expect(
        reservationManagementService.formatModificationType('PET_ADDED')
      ).toBe('Pet Added');

      expect(
        reservationManagementService.formatModificationType('PET_REMOVED')
      ).toBe('Pet Removed');

      expect(
        reservationManagementService.formatModificationType('ADDON_ADDED')
      ).toBe('Add-on Added');

      expect(
        reservationManagementService.formatModificationType('ADDON_REMOVED')
      ).toBe('Add-on Removed');

      expect(
        reservationManagementService.formatModificationType('SERVICE_CHANGE')
      ).toBe('Service Changed');

      expect(
        reservationManagementService.formatModificationType('NOTES_UPDATED')
      ).toBe('Notes Updated');

      expect(
        reservationManagementService.formatModificationType('CANCELLED')
      ).toBe('Cancelled');
    });

    it('should return original type for unknown types', () => {
      expect(
        reservationManagementService.formatModificationType('UNKNOWN_TYPE')
      ).toBe('UNKNOWN_TYPE');
    });
  });
  ...
  describe('getStatusColor', () => {
    it('should return correct colors for each status', () => {
      expect(reservationManagementService.getStatusColor('PENDING')).toBe(
        'warning'
      );
      expect(reservationManagementService.getStatusColor('CONFIRMED')).toBe(
        'success'
      );
      expect(reservationManagementService.getStatusColor('CHECKED_IN')).toBe(
        'info'
      );
      expect(reservationManagementService.getStatusColor('CHECKED_OUT')).toBe(
        'primary'
      );
      expect(reservationManagementService.getStatusColor('CANCELLED')).toBe(
        'error'
      );
      expect(reservationManagementService.getStatusColor('COMPLETED')).toBe(
        'default'
      );
      expect(reservationManagementService.getStatusColor('NO_SHOW')).toBe(
        'error'
      );
    });
  });
  ...
  describe('getStatusLabel', () => {
    it('should return correct labels for each status', () => {
      expect(reservationManagementService.getStatusLabel('PENDING')).toBe(
        'Pending'
      );
      expect(reservationManagementService.getStatusLabel('CONFIRMED')).toBe(
        'Confirmed'
      );
      expect(reservationManagementService.getStatusLabel('CHECKED_IN')).toBe(
        'Checked In'
      );
      expect(reservationManagementService.getStatusLabel('CHECKED_OUT')).toBe(
        'Checked Out'
      );
      expect(reservationManagementService.getStatusLabel('CANCELLED')).toBe(
        'Cancelled'
      );
      expect(reservationManagementService.getStatusLabel('COMPLETED')).toBe(
        'Completed'
      );
      expect(reservationManagementService.getStatusLabel('NO_SHOW')).toBe(
        'No Show'
      );
    });
  });
  ...
  describe('Business Rules', () => {
    it('should enforce 24-hour modification window', () => {
      ...
      expect(
        reservationManagementService.canModifyReservation(reservation)
      ).toBe(false);
    });
