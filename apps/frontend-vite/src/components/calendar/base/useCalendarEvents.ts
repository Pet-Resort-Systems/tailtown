...
export function useCalendarEvents(options: UseCalendarEventsOptions = {}) {
   ...
   */
  const defaultTransformEvent = useCallback(
    (reservation: any): CalendarEvent => {
      return {
        id: reservation.id,
  ...
  const loadEvents = useCallback(
    async (date?: Date) => {
      try {
        ...
        // Build status filter
        const statusFilter =
          options.statusFilter || 'PENDING,CONFIRMED,CHECKED_IN';
        ...
        if (
          ...
          if (
            ...
            options.serviceCategories.length > 0
          ) {
            filteredReservations = filteredReservations.filter(
              (reservation: any) => {
                ...
                return (
                  options.serviceCategories?.some(
                    (category) => serviceObj.serviceCategory === category
                  ) || false
                );
              }
            );
