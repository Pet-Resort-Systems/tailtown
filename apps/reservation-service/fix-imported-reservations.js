...
async function fixReservations() {
  try {
    ...
    console.log(
      `Found ${reservationsWithoutResource.length} reservations without resourceId`
    );
    ...
    if (!defaultResource) {
      console.error(
        '❌ No default resource found! Please create at least one STANDARD_SUITE resource.'
      );
      process.exit(1);
    }

    console.log(
      `Using default resource: ${defaultResource.name} (${defaultResource.id})`
    );
