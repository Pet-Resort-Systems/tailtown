...
async function main() {
  try {
    ...
    const migrationPathArg = args.find((arg) =>
      arg.startsWith('--migration-path=')
    );
    ...
    console.log(
      `Tables with Missing Columns: ${Object.keys(validationResult.missingColumns).length}`
    );
    console.log(
      `Tables with Missing Indexes: ${Object.keys(validationResult.missingIndexes).length}`
    );
    console.log(
      `Tables with Missing Relationships: ${Object.keys(validationResult.missingRelationships).length}`
    );
    ...
    if (validationResult.report.migrationScript) {
      ...
      console.log('---------------------------------------');
      console.log(
        validationResult.report.migrationScript.substring(0, 500) + '...'
      );
    }
