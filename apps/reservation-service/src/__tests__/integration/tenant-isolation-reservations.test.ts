...
describe('Reservation Tenant Isolation', () => {
  ...
  beforeAll(async () => {
    ...
    // Create pets
    const pA = await prisma.pet.create({
      data: {
        name: 'Pet A',
        type: 'DOG',
    ...
    petAId = pA.id;
    const pB = await prisma.pet.create({
      data: {
        name: 'Pet B',
        type: 'DOG',
    ...
    // Create resources
    const rA = await prisma.resource.create({
      data: {
        name: 'Resource A',
        type: 'KENNEL',
        capacity: 1,
    ...
    resourceAId = rA.id;
    const rB = await prisma.resource.create({
      data: {
        name: 'Resource B',
        type: 'KENNEL',
        capacity: 1,
