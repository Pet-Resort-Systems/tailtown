...
jest.mock('../../utils/reservation-conflicts', () => ({
  detectReservationConflicts: (...args: any[]) =>
    mockDetectReservationConflicts(...args),
}));

// Mock the safeExecutePrismaQuery function
const safeExecutePrismaQueryMock = jest
  .fn()
  .mockImplementation((fn, fallback) => fn());
  ...
  describe('createReservation', () => {
    ...
    it('should return conflict error when resource has conflicts', async () => {
      ...
      mockDetectReservationConflicts.mockResolvedValue({
        ...
        conflictingReservations: [{ id: 'conflict-1' }],
        warnings: [
          'Resource is not available for the requested dates. There are 1 overlapping reservations.',
        ],
      });
    ...
    it('should return conflict error when pet has conflicts', async () => {
      ...
      mockDetectReservationConflicts.mockResolvedValue({
        ...
        conflictingReservations: [{ id: 'conflict-1' }],
        warnings: [
          'Pet already has 1 overlapping reservation(s) during the requested dates.',
        ],
      });
