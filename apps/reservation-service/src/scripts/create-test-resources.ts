...
async function createTestResources() {
  ...
  const suiteTypes = [
    { type: 'STANDARD_SUITE', count: 5, displayName: 'Standard Suite' },
    {
      ...
      count: 3,
      displayName: 'Standard Plus Suite',
    },
  ...
  try {
    ...
    if (existingCount > 0) {
      console.log(
        `Found ${existingCount} existing resources for tenant ${tenantId}.`
      );
    ...
    console.log(
      `Successfully created ${createdCount} resources for tenant ${tenantId}!`
    );
