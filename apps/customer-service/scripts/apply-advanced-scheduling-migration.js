...
async function applyMigration() {
  ...
  try {
    ...
    const sqlPath = path.join(
      __dirname,
      '../prisma/migrations/add_advanced_scheduling.sql'
    );
        ...
        try {
          ...
          if (
            ...
            process.stdout.write(
              `  ⚠ Statement ${i + 1}/${statements.length} (already exists)\r`
            );
          } else {
    ...
    console.log(
      '  - Added grooming_skills, max_appointments_per_day, average_service_time to staff table'
    );
