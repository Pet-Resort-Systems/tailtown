...
async function checkDates() {
  try {
    ...
    console.log(
      `Reservations this week (${startOfWeek.toDateString()} - ${endOfWeek.toDateString()}): ${thisWeekReservations}`
    );
    ...
    recentReservations.forEach((res, index) => {
      console.log(
        `${index + 1}. ${res.customer.firstName} ${res.customer.lastName} - ${res.service.name}`
      );
      console.log(
        `   Date: ${res.startDate.toDateString()} - ${res.endDate.toDateString()}`
      );
    ...
    console.log(
      `Invoice date range: ${invoiceDateRange._min.issueDate} to ${invoiceDateRange._max.issueDate}`
    );
  } catch (error) {
