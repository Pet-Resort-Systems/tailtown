...
async function testRevenueConsistency() {
  ...
  try {
    ...
    const serviceRevenue =
      await financialService.getServiceRevenue(TEST_DATE_RANGE);
    const serviceTotal = serviceRevenue.reduce(
    ...
    // Get add-on revenue
    const addOnRevenue =
      await financialService.getAddOnRevenue(TEST_DATE_RANGE);
    const addOnTotal = addOnRevenue.reduce(
    ...
    if (serviceDiff > 0.01) {
      console.error(
        '❌ INCONSISTENCY DETECTED: Service + Add-On total does not match financial summary'
      );
    } else {
      console.log(
        '✅ Service + Add-On total consistent with financial summary'
      );
    }

    // Get customer revenue
    const customerRevenue =
      await financialService.getCustomerRevenue(TEST_DATE_RANGE);
    const customerTotal = customerRevenue.reduce(
    ...
    if (customerDiff > 0.01) {
      console.error(
        '❌ INCONSISTENCY DETECTED: Customer total does not match financial summary'
      );
    } else {
    ...
    const dailyRevenue =
      await financialService.getDailyRevenue(TEST_DATE_RANGE);
    ...
    if (dailyDiff > 0.01) {
      console.error(
        '❌ INCONSISTENCY DETECTED: Daily total does not match financial summary'
      );
    } else {
