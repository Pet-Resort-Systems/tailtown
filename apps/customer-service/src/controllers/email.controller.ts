...
export class EmailController {
  ...
  async getEmailConfig(req: TenantRequest, res: Response, next: NextFunction) {
    try {
      const isConfigured = !!env.CUSTOMER_SENDGRID_API_KEY;

      res.json({
        success: true,
        data: {
          isConfigured,
          fromEmail: env.CUSTOMER_SENDGRID_FROM_EMAIL || 'noreply@tailtown.com',
          fromName: env.CUSTOMER_SENDGRID_FROM_NAME || 'Tailtown Pet Resort',
          provider: 'SendGrid',
        },
      });
    } catch (error) {
