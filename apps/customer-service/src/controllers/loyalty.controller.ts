...
export const redeemPoints = async (
  ...
  try {
    ...
    res
      .status(201)
      .json({ status: 'success', data: { redemption, member: updatedMember } });
  } catch (error) {
    next(error);
  }
};
...
export const getMemberStats = async (
  ...
  try {
    ...
    const totalEarned = member.transactions.reduce(
      (sum, t) => sum + t.points,
      0
    );
    const totalRedeemed = member.redemptions.reduce(
      (sum, r) => sum + r.pointsRedeemed,
      0
    );
    const nextTier = getNextTier(member.currentTier);
    const pointsToNextTier = nextTier
      ? getTierMinPoints(nextTier) - member.lifetimePoints
      : 0;
