...
export const getDashboardSummary = async (
  ...
  try {
    ...
    console.log(
      'Fixed Analytics: Getting dashboard summary with period:',
      period
    );
    ...
    console.log(
      `Fixed Analytics: Dashboard summary - Revenue: ${invoiceTotal._sum.total || 0}, Customers: ${customerCount}, Services: ${serviceData.length}, Add-ons: ${addOnData.length}, Reservations: ${reservations.length}`
    );

    // If no data for current period, add a note
    const hasCurrentData =
      reservations.length > 0 || (invoiceTotal._sum.total || 0) > 0;
    ...
    res.status(200).json({
      ...
      data: {
        ...
        message: hasCurrentData
          ? null
          : `No data found for ${period === 'month' ? 'current month' : period}. Try selecting 'All Time' to see historical data.`,
      },
    });
  } catch (error) {
...
export const getSalesByService = async (
  ...
  try {
    ...
    console.log(
      'Fixed Analytics: Getting sales by service with period:',
      period
    );
    ...
    console.log(
      `Fixed Analytics: Found ${reservations.length} reservations for the period`
    );
    ...
    for (const reservation of reservations) {
      const serviceId = reservation.serviceId;
      const service =
        reservation.service || allServices.find((s) => s.id === serviceId);
    ...
    const totalRevenue = salesByService.reduce(
    ...
    const servicesWithPercentages = salesByService.map((service) => ({
      ...
      percentage:
        totalRevenue > 0
          ? parseFloat(((service.revenue / totalRevenue) * 100).toFixed(1))
          : 0,
    }));

    console.log(
      `Fixed Analytics: Found ${salesByService.length} services with total revenue ${totalRevenue}`
    );
    ...
    res.status(200).json({
      ...
      data: {
        ...
        message: hasCurrentData
          ? null
          : `No reservations found for ${period === 'month' ? 'current month' : period}. Try selecting 'All Time' to see historical data.`,
      },
    });
  } catch (error) {
...
export const getSalesByAddOn = async (
  ...
  try {
    ...
    console.log(
      'Fixed Analytics: Getting sales by add-on with period:',
      period
    );
    ...
    // Calculate total revenue
    const totalRevenue = salesByAddOn.reduce(
      (sum, addOn) => sum + addOn.revenue,
      0
    );

    console.log(
      `Fixed Analytics: Found ${salesByAddOn.length} add-ons with total revenue ${totalRevenue}`
    );
  ...
  try {
    ...
    const customerValues = customers
      .map((customer) => {
        ...
        const addOnTotal = Array.from(addOnMap.values()).reduce(
          (sum, addOn) => sum + addOn.revenue,
          0
        );
    ...
    res.status(200).json({
      ...
      data: {
        ...
        transactions: customer.reservations.map((reservation) => ({
          ...
          addOns:
            reservation.addOnServices?.map((addOn) => ({
