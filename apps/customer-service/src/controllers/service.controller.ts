  ...
  try {
    ...
    const service = await prisma.service.create({
      data: {
        ...
        serviceCategory: mainServiceData.serviceCategory,
        isActive:
          mainServiceData.isActive !== undefined
            ? mainServiceData.isActive
            : true,
        ...
        taxable:
          mainServiceData.taxable !== undefined
            ? mainServiceData.taxable
            : true,
      },
    });
...
export const updateService = async (
  ...
  try {
    ...
    logger.debug('Updating service', {
      tenantId: req.tenantId,
      serviceId: id,
      fields: Object.keys(mainServiceData),
    });
...
export const deleteService = async (
  ...
  try {
    ...
    if (activeReservationsCount > 0 && !forceDelete) {
      logger.info('Service has active reservations, performing soft delete', {
        tenantId: req.tenantId,
        serviceId: id,
      ...
      await deleteCache(cacheKey);
      await deleteCachePattern(`${req.tenantId}:services:*`);
      logger.debug('Service caches invalidated after soft delete', {
        tenantId: req.tenantId,
        serviceId: id,
      });
      ...
      return res.status(200).json({
        status: 'success',
        message:
          'Service has been deactivated (soft deleted) because it has active reservations',
      });
    }
