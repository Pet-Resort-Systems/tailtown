  ...
  try {
    ...
    const parsed = templates.map((t) => ({
      ...
      requiredForCompletion: t.requiredForCompletion
        ? JSON.parse(t.requiredForCompletion)
        : [],
    }));
  ...
  try {
    ...
    const parsed = {
      ...
      requiredForCompletion: template.requiredForCompletion
        ? JSON.parse(template.requiredForCompletion)
        : [],
    };
    ...
    const template = await prisma.checklistTemplate.create({
      data: {
        ...
        requiredForCompletion: requiredForCompletion
          ? JSON.stringify(requiredForCompletion)
          : null,
      },
    });
...
export const updateTemplate = async (
  ...
  try {
    ...
    if (requiredForCompletion)
      updateData.requiredForCompletion = JSON.stringify(requiredForCompletion);
...
export const getChecklistInstance = async (
  ...
  try {
    const id = assertStringRouteParam(
      ...
      req.originalUrl,
      AppError.validationError,
      'Checklist instance ID is required'
    );
    const tenantId =
      req.tenantId || (env.NODE_ENV !== 'production' && 'dev');
...
export const updateChecklistItem = async (
  ...
  try {
    ...
    const itemIndex = items.findIndex(
      (i: any) => i.templateItemId === templateItemId
    );
  ...
  try {
    ...
    if (completedInstances.length > 0) {
      ...
      averageCompletionTime = Math.round(
        totalTime / completedInstances.length / 60000
      ); // minutes
    }
