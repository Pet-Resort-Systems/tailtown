...
export const getAvailabilityCalendar = async (
  ...
  try {
    ...
    while (currentDate <= endDate) {
      ...
      const occupiedSuites = reservations
        .filter((res) => {
...
export const getAlternativeDates = async (
  ...
  try {
    ...
    for (let offset = 1; offset <= Number(daysToCheck); offset++) {
      ...
      const beforeReservations = allReservations.filter(
        (res) => res.startDate <= beforeEnd && res.endDate >= beforeStart
      );

      const beforeAvailable = resources.filter(
        (r) => !beforeReservations.some((res) => res.resourceId === r.id)
      );
      ...
      const afterReservations = allReservations.filter(
        (res) => res.startDate <= afterEnd && res.endDate >= afterStart
      );

      const afterAvailable = resources.filter(
        (r) => !afterReservations.some((res) => res.resourceId === r.id)
      );
