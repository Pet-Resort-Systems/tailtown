...
export const getAllSuites = async (
  ...
  try {
    ...
    if (date && typeof date === 'string') {
      ...
      console.log(
        `Parsed date from string: ${date} -> ${filterDate.toISOString()}`
      );
    ...
    console.log(
      `Filtering with date range: ${filterTodayStart.toISOString()} to ${filterTodayEnd.toISOString()}`
    );
    console.log(
      `Filter params: type=${type}, status=${status}, available=${available}`
    );

    // Filter by status (AVAILABLE, OCCUPIED, MAINTENANCE)
    if (status) {
      console.log(
        `Filtering by status: ${status} for date ${filterTodayStart.toISOString().split('T')[0]}`
      );

      if (status === 'OCCUPIED') {
        console.log(
          'Applying OCCUPIED filter - looking for suites with active reservations'
        );
    ...
    if (available === 'true') {
      ...
      console.log(
        'Backward compatibility available=true filter where clause:',
        JSON.stringify(where, null, 2)
      );
    }
    ...
    console.log(
      `getAllSuites - Today's range: ${todayStart.toISOString()} to ${todayEnd.toISOString()}`
    );
...
export const initializeSuites = async (
  ...
  try {
    ...
    if (vipCount + standardPlusCount > count) {
      return next(
        new AppError(
          'Sum of VIP and Standard Plus suites exceeds total count',
          400
        )
      );
    }
...
export const getSuiteStats = async (
  ...
  try {
    ...
    if (date && typeof date === 'string') {
      ...
      console.log(
        `Stats - Parsed date from string: ${date} -> ${statsDate.toISOString()}`
      );
      // Check if date is valid
    ...
    console.log(
      `Stats - Date range: ${statsTodayStart.toISOString()} to ${statsTodayEnd.toISOString()}`
    );
    ...
    const [totalSuites, occupiedSuites, maintenanceSuites, availableSuites] =
      await Promise.all([
        // Total count
    ...
    const occupancyRate =
      totalSuites > 0 ? (occupiedSuites / totalSuites) * 100 : 0;
