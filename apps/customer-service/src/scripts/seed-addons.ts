...
async function main() {
  ...
  try {
    ...
    if (services.length === 0) {
      console.log(
        'No services found in the database. Please seed services first.'
      );
      return;
    }
    ...
    const boardingServices = services.filter(
      (s) => s.serviceCategory === 'BOARDING'
    );
    const daycareServices = services.filter(
      (s) => s.serviceCategory === 'DAYCARE'
    );
    const groomingServices = services.filter(
      (s) => s.serviceCategory === 'GROOMING'
    );

    // Log service IDs for reference
    console.log(
      'Boarding service IDs:',
      boardingServices.map((s) => s.id)
    );
    console.log(
      'Daycare service IDs:',
      daycareServices.map((s) => s.id)
    );
    console.log(
      'Grooming service IDs:',
      groomingServices.map((s) => s.id)
    );
    ...
    if (boardingServices.length > 0) {
      ...
      for (const addOn of boardingAddOns) {
        ...
        if (!existingAddOn) {
          ...
          console.log(
            `Created boarding add-on: ${created.name} with ID: ${created.id}`
          );
        } else {
          console.log(
            `Boarding add-on ${existingAddOn.name} already exists with ID: ${existingAddOn.id}`
          );
        }
      }
    }
    ...
    if (groomingServices.length > 0) {
      ...
      for (const addOn of groomingAddOns) {
        ...
        if (!existingAddOn) {
          ...
          console.log(
            `Created grooming add-on: ${created.name} with ID: ${created.id}`
          );
        } else {
          console.log(
            `Grooming add-on ${existingAddOn.name} already exists with ID: ${existingAddOn.id}`
          );
        }
      }
    }
