...
async function addGroomingAddOns() {
  try {
    ...
    for (const service of groomingServices) {
      console.log(
        `Adding add-ons to service: ${service.name} (ID: ${service.id})`
      );
      ...
      if (existingAddOns.length > 0) {
        console.log(
          `Service ${service.name} already has ${existingAddOns.length} add-ons. Skipping.`
        );
        continue;
      }
      ...
      for (const addOn of groomingAddOns) {
        ...
        console.log(
          `Added add-on "${addOn.name}" to service "${service.name}"`
        );
      }
    }
