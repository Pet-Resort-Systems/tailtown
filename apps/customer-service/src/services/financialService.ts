...
const INVALID_INVOICE_STATUSES = [
  'DRAFT',
  'CANCELLED',
  'REFUNDED',
] as InvoiceStatus[];
...
const INVALID_RESERVATION_STATUSES = [
  'CANCELLED',
  'NO_SHOW',
  'PENDING',
] as ReservationStatus[];
...
export async function getServiceRevenue(
  ...
  for (const invoice of invoices) {
    if (invoice.reservation?.service) {
      ...
      const addOnTotal =
        invoice.reservation.addOnServices?.reduce(
...
export async function getAddOnRevenue(
  ...
  // Calculate total revenue to determine percentages
  const totalRevenue = addOnRevenueData.reduce(
...
export async function getCustomerRevenue(
  ...
  for (const [_, customerData] of customerMap.entries()) {
    ...
    const serviceTotal = serviceRevenues.reduce(
      (sum, service) => sum + service.revenue,
      0
    );
    ...
    const addOnTotal = addOnRevenues.reduce(
      (sum, addOn) => sum + addOn.revenue,
      0
    );
