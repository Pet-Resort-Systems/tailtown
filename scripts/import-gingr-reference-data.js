...
async function gingrPostRequest(endpoint, data = {}) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Gingr API error: ${response.status} ${response.statusMessage}\n${text}`
    );
  }
...
async function fetchAllReferenceData() {
  ...
  try {
    ...
    const response = await gingrGetRequest('/reservation_types', {
      active_only: true,
    });
    data.reservationTypes =
      response.reservation_types || response.data || response;
    if (!Array.isArray(data.reservationTypes)) {
      data.reservationTypes = Object.values(data.reservationTypes);
    }
    console.log(
      `     ✅ Found ${data.reservationTypes.length} reservation types\n`
    );
  } catch (error) {
    console.log(`     ❌ Error: ${error.message}\n`);
    data.reservationTypes = [];
  ...
  if (data.species && data.species.length > 0) {
    ...
    for (const species of data.species) {
      try {
        const response = await gingrGetRequest('/get_immunization_types', {
          species_id: species.id,
        });
        let immunizations =
          response.immunization_types ||
        ...
        const speciesName =
          species.name || species.species_name || `Species ${species.id}`;
        data.immunizations[speciesName] = immunizations;
        console.log(
          `     ✅ ${speciesName}: ${immunizations.length} immunization types`
        );
      } catch (error) {
        const speciesName =
          species.name || species.species_name || `Species ${species.id}`;
...
function displaySummary(data) {
  ...
  if (data.breeds && data.breeds.length > 0) {
    ...
    data.breeds.forEach((breed) => {
      ...
      breedsBySpecies[speciesName].push(
        breed.name || breed.breed_name || breed.breed || JSON.stringify(breed)
      );
    });
  ...
  if (data.species && data.species.length > 0) {
    ...
    data.species.forEach((species) => {
      const name =
        species.name || species.species_name || JSON.stringify(species);
...
function generateRecommendations(data) {
  ...
  console.log(
    '   • Import into Tailtown as dropdown options for pet breed field'
  );
  ...
  console.log(
    '   • Current Tailtown types: DOG, CAT, BIRD, REPTILE, SMALL_MAMMAL, OTHER'
  );
  ...
  console.log(
    '   • Consider service categories (Boarding, Daycare, Grooming, etc.)\n'
  );
